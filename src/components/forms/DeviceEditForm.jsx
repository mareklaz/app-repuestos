...
export default function DeviceEditForm({ device }) {
  ...
  useEffect(() => {
    const formattedDevice = {
      ...
      warranty: {
        ...device.warranty,
        start: device.warranty?.start
          ? formatDate(new Date(device.warranty.start))
          : undefined,
        end: device.warranty?.end
          ? formatDate(new Date(device.warranty.end))
          : undefined,
      },
    };
    reset(formattedDevice);
  }, [device, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await updateDevice({ ...data, id: device.id });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-6 sm:space-y-8'>
      ...
      <div className='flex items-center justify-end gap-x-6'>
        <Button type='submit' variant='primary' disabled={loading}>
          {loading ? 'Procesando...' : 'Actualizar Dispositivo'}
        </Button>
      </div>
    </form>
  );
